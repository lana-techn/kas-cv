...
// --- Perbaikan Query SQL ---
// Menggunakan nama tabel yang benar: 'penjualan', 'pembelian', 'biaya'
$stmt = $pdo->query("SELECT SUM(total_jual) as total_penjualan FROM penjualan");
$total_penjualan = $stmt->fetch(PDO::FETCH_ASSOC)['total_penjualan'] ?? 0;

$stmt = $pdo->query("SELECT SUM(total_beli) as total_pembelian FROM pembelian");
$total_pembelian = $stmt->fetch(PDO::FETCH_ASSOC)['total_pembelian'] ?? 0;

$stmt = $pdo->query("SELECT SUM(total) as total_biaya FROM biaya");
$total_biaya = $stmt->fetch(PDO::FETCH_ASSOC)['total_biaya'] ?? 0;

$saldo_kas = $total_penjualan - ($total_pembelian + $total_biaya);

$stmt = $pdo->query("SELECT * FROM barang ORDER BY stok ASC");
...
<script>
// --- Membungkus Chart.js dengan event listener ---
// Memastikan skrip berjalan setelah seluruh dokumen HTML dimuat
